...
import { searchErratas } from "./action";
...
export default function ErratasClientView({
  ...
  const [erratas, setErratas] = useState(initialErratas);
  ...
  const fetchErratas = (params: {
    ...
    startTransition(async () => {
      const result = await searchErratas({
      ...
      setErratas(result.erratas);
      setTotalCount(result.totalCount);
      setCurrentPage(params.page);

      // Sync page number in URL
      const params2 = new URLSearchParams(urlSearchParams.toString());
      if (params.page === 1) {
        params2.delete("page");
      } else {
        params2.set("page", String(params.page));
      }
  ...
  const handleSearchChange = (value: string) => {
    ...
    searchTimerRef.current = setTimeout(() => {
      fetchErratas({ search: value, type: typeFilter, sortOrder, page: 1 });
    }, 400);
  };

  const handleTypeChange = (value: ErrataType | "all") => {
    setTypeFilter(value);
    fetchErratas({ search: searchQuery, type: value, sortOrder, page: 1 });
  };

  const toggleSortOrder = () => {
    const newOrder = sortOrder === "desc" ? "asc" : "desc";
    setSortOrder(newOrder);
    fetchErratas({ search: searchQuery, type: typeFilter, sortOrder: newOrder, page: 1 });
  };

  const goToPage = (page: number) => {
    fetchErratas({ search: searchQuery, type: typeFilter, sortOrder, page });
  };
    ...
    <>
      ...
      <div className="mb-6 space-y-4">
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          {/* Filtre par type */}
          <Select value={typeFilter} onValueChange={(v) => handleTypeChange(v as ErrataType | "all")}>
