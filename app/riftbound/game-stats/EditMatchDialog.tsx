...
import { Match, MatchCard, MatchGame, GameResult } from "@/lib/types/match";
import { updateMatchAction } from "@/app/riftbound/game-stats/action";
import CardSearchInput from "@/app/riftbound/game-stats/CardSearchInput";
import { Trash2, Trophy, Sword } from "lucide-react";
...
export default function EditMatchDialog({
  ...
  const [userLegend, setUserLegend] = useState<MatchCard | null>(match.userLegend);
  const [opponentName, setOpponentName] = useState(match.opponentName ?? "");
  const [opponentLegend, setOpponentLegend] = useState<MatchCard | null>(match.opponentLegend);
  const [games, setGames] = useState<MatchGame[]>(match.games);
  const [notes, setNotes] = useState(match.notes ?? "");
  const [matchDate, setMatchDate] = useState(match.matchDate.split("T")[0]);
  ...
  function resetToOriginal() {
    setUserLegend(match.userLegend);
    setOpponentName(match.opponentName ?? "");
    setOpponentLegend(match.opponentLegend);
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            ...
            <DialogDescription>
              Modifiez les informations et résultats du match.
            </DialogDescription>
          </DialogHeader>
