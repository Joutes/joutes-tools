...
import {removeCardFromBoosterAction} from "@/app/collection/boosters/action";

function StarIconFull(props: { className: string }) {
  return null;
}

function StarIconEmpty(props: { className: string }) {
  return null;
}

export default function CardCard({ card, boosterId }: { boosterId: Booster['id'], card: BoosterCard }) {
  return (
    <div key={card.id} className={`rounded-md border-2 ${card.foil ? 'border-yellow-500' : 'border-gray-200'} p-2 w-[160px] flex flex-col`}>
      ...
      <div>
        <button onClick={async () => {
          await removeCardFromBoosterAction(boosterId, card.id);
        }}>
